...
func (t *Torrent) Download() {
	fmt.Printf("Downloading %s", t.Title)
	response, err := http.Get(t.Href)
	if err != nil {
		fmt.Printf("Failed to download %s", t.Title)
		return
	}
	defer response.Body.Close()
	contents, err := ioutil.ReadAll(response.Body)
	if err != nil {
		fmt.Printf("Failed to read the contents of %s", t.Title)
		return
	}

	if err := ioutil.WriteFile(t.filename(), contents, 0644); err != nil {
		fmt.Printf("Failed to save the contents of %s to the torrent file", t.Title)
	}
}
