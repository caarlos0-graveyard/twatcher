...
func (t *Torrent) Download() {
	...
	if err != nil {
		log.WithError(err).WithField("title", t.Title).
			Errorln("Failed to open torrent!")
		return
	}

	if err := ioutil.WriteFile(t.filename(), contents, 0644); err != nil {
		log.WithError(err).WithField("title", t.Title).
			Errorln("Failed to write torrent to folder!")
		return
	}
}
...
func (t *Torrent) filename() string {
	...
	if err != nil {
		log.WithError(err).WithField("title", t.Title).
			Errorln("Failed to get current user! Will save files in /tmp.")
